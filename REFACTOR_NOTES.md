...
- Worker B: `sap-runtime-license-trap.md` has a `redirectTo` frontmatter field added but `build.js` does not implement redirect logic. Worker A should either handle the redirect (301 via meta-refresh or JS) or Worker C can add it as part of link cleanup. The old slug page should not 404 after Worker C's link audit.
- Worker B: The `reference-starter` collapsible in `walkthrough.html` uses `{{#isAdf}}` etc. Mustache flags that `build.js` does not currently populate. Worker A should add these flags from the walkthrough's `extractors` frontmatter list (map 'ADF'→isAdf, 'Databricks'→isDatabricks, etc.) — or Worker B can switch to a simpler approach (no per-tool doc link, just a generic vendor docs disclaimer) if Worker A does not address this.
- Worker B: The `method` frontmatter field added to walkthroughs (e.g., `method: odp`) is not currently consumed by `build.js`. Worker A should pass it through to the template context so `{{#method.odp}}` Mustache sections in `license-check.mustache` work correctly at build time (currently the JS client-side fallback handles this).
