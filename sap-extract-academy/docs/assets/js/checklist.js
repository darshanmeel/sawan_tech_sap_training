...
(function() {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { initializeChecklist(); showPdfButton(); });
  } else {
    initializeChecklist();
    showPdfButton();
  }
  ...
  document.addEventListener('click', (event) => {
    if (event.target.closest('#reset-progress')) clearChecklist();
    if (event.target.closest('#download-pdf')) window.print();
  });
})();
